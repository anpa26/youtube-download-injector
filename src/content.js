...
(function() {
  ...
  const styles = `
    ...
    .yt-downloader-dropdown.theme-pitch-black,
    .yt-sub-modal.theme-pitch-black {
    ...
    .yt-sub-modal.theme-dark select option,
    .yt-downloader-dropdown.theme-pitch-black select option,
    .yt-sub-modal.theme-pitch-black select option {
      background-color: #1f1f2e !important;
      color: #ffffff !important;
    }
    .yt-downloader-dropdown.theme-light select option,
    .yt-sub-modal.theme-light select option {
  ...
  const toastStyles = `
    ...
    .yt-dl-toast.yt-dl-toast--show {
      display: flex !important;
      flex-direction: column !important;
    }
    .yt-dl-toast.yt-dl-toast--hide {
      display: none !important;
    }
    .yt-dl-toast-inner {
      display: flex !important;
    ...
    .yt-dl-toast-thumb-placeholder {
      width: 36px !important;
      height: 36px !important;
      border-radius: 50% !important;
      flex-shrink: 0 !important;
      background: linear-gradient(135deg, rgba(255,62,62,0.3), rgba(255,62,62,0.1)) !important;
      display: flex !important;
      align-items: center !important;
      justify-content: center !important;
    }
    ...
    .yt-dl-toast-close:hover {
      color: #fff !important;
      background: rgba(255,62,62,0.2) !important;
    }
    .yt-dl-toast-progress-bar {
    ...
    .yt-dl-toast--success .yt-dl-toast-progress-bar {
      background: #22c55e !important;
    }
    .yt-dl-toast--error .yt-dl-toast-progress-bar {
  ...
  styleEl.textContent = styles + toastStyles;
  ...
  function getOrCreateToast() {
    ...
    _toastEl.className = 'yt-dl-toast';
    _toastEl.innerHTML = `
      <div class="yt-dl-toast-inner">
        <div class="yt-dl-toast-thumb-placeholder" id="yt-dl-thumb-wrap">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#ff3e3e" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        </div>
        <div class="yt-dl-toast-content">
          <div class="yt-dl-toast-title" id="yt-dl-title">Downloading...</div>
          <div class="yt-dl-toast-status-row">
            <span class="yt-dl-toast-status" id="yt-dl-status">Starting...</span>
            <span class="yt-dl-toast-pct" id="yt-dl-pct">0%</span>
          </div>
        </div>
        <button class="yt-dl-toast-close" id="yt-dl-close" title="Cancel">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
      </div>
      <div class="yt-dl-toast-progress-bar" id="yt-dl-bar" style="width:0%"></div>
    `;

    document.body.appendChild(_toastEl);

    _toastBarEl = _toastEl.querySelector('#yt-dl-bar');
    _toastStatusEl = _toastEl.querySelector('#yt-dl-status');
    _toastPctEl = _toastEl.querySelector('#yt-dl-pct');
    _toastCancelBtn = null;

    _toastEl.querySelector('#yt-dl-close').addEventListener('click', () => {
      if (_toastCurrentTaskId) {
        chrome.runtime.sendMessage({ action: 'cancel_download', taskId: _toastCurrentTaskId }).catch(() => {});
        const controller = activeFetches[_toastCurrentTaskId];
        if (controller) {
          controller.abort();
          delete activeFetches[_toastCurrentTaskId];
        }
      }
      hideToast(true);
    });
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        _toastEl.classList.add('yt-dl-toast--show');
      });
    });
  ...
  function showToast(taskId, title, thumbnailUrl) {
    if (_toastHideTimer) {
    ...
    // Reset state
    toast.classList.remove('yt-dl-toast--success', 'yt-dl-toast--error', 'yt-dl-toast--hide');
    toast.classList.add('yt-dl-toast--show');

    const titleEl = toast.querySelector('#yt-dl-title');
    if (titleEl) titleEl.textContent = title || 'Downloading...';

    const thumbWrap = toast.querySelector('#yt-dl-thumb-wrap');
    if (thumbWrap && thumbnailUrl) {
      thumbWrap.innerHTML = `<img class="yt-dl-toast-thumb" src="${thumbnailUrl}" onerror="this.style.display='none'">`;
    }

    if (_toastCancelBtn) _toastCancelBtn.style.display = '';
    if (_toastBarEl) {
      _toastBarEl.classList.add('yt-dl-toast-bar--pulse');
      _toastBarEl.style.width = '0%';
    }
    if (_toastPctEl) _toastPctEl.textContent = '0%';
  }

  function updateToastProgress(progress, statusText) {
    if (!_toastEl) return;
    if (_toastBarEl) {
      _toastBarEl.classList.remove('yt-dl-toast-bar--pulse');
      _toastBarEl.style.width = `${Math.min(100, progress)}%`;
    }
    if (_toastPctEl) _toastPctEl.textContent = `${Math.min(100, progress)}%`;
    if (_toastStatusEl && statusText) _toastStatusEl.textContent = statusText;
  }

  function finishToast(success, message) {
    if (!_toastEl) return;
    _toastEl.classList.remove('yt-dl-toast--success', 'yt-dl-toast--error');
    _toastEl.classList.add(success ? 'yt-dl-toast--success' : 'yt-dl-toast--error');
    if (_toastBarEl) {
      _toastBarEl.classList.remove('yt-dl-toast-bar--pulse');
      _toastBarEl.style.width = success ? '100%' : _toastBarEl.style.width;
    }
    ...
    if (_toastCancelBtn) _toastCancelBtn.style.display = 'none';
  ...
  function hideToast(immediate) {
    ...
    _toastEl.classList.remove('yt-dl-toast--show');
    _toastEl.classList.add('yt-dl-toast--hide');
    setTimeout(() => {
      if (_toastEl) { _toastEl.remove(); _toastEl = null; }
      _toastBarEl = null; _toastStatusEl = null; _toastPctEl = null; _toastCancelBtn = null;
    }, immediate ? 0 : 400);
  }
  ...
  function getStatusText(status, progress) {
    switch (status) {
      ...
      case 'muxing': return 'Muxing tracks...';
      case 'converting': return 'Converting to MP3...';
      case 'embedding': return 'Finalizing...';
      case 'success': return 'Download complete!';
      case 'failed': return 'Download failed';
      default: return status || `Downloading... ${progress}%`;
    }
  }
  // ────────────────────────────────────────────────────────────────────────────

  // ─── MP3 Conversion via lame.js ─────────────────────────────────────────────
  async function convertM4aToMp3(m4aBuffer, signal, onProgress) {
    // lame.js (lamejs) should be loaded as global via content_scripts
    const lamejsLib = window.lamejs || self.lamejs || globalThis.lamejs;
    if (!lamejsLib) throw new Error('lamejs not loaded');

    // We need to decode the audio first using Web Audio API, then encode to MP3
    ...
    try {
      ...
      audioBuffer = await audioCtx.decodeAudioData(m4aBuffer.slice(0));
    } catch (e) {
      audioCtx.close();
      throw e;
    }
    ...
    const mp3Encoder = new lamejsLib.Mp3Encoder(numChannels, sampleRate, bitRate);
    ...
    for (let i = 0; i < totalSamples; i += blockSize) {
      ...
      const leftSlice = leftChannel.slice(i, i + blockSize);
      const rightSlice = rightChannel.slice(i, i + blockSize);
      const leftInt16 = floatToInt16(leftSlice);
      const rightInt16 = floatToInt16(rightSlice);
      ...
      processedSamples += leftSlice.length;
      if (onProgress) {
        const pct = 25 + Math.round((processedSamples / totalSamples) * 70);
    ...
    // Combine all MP3 chunks into one Uint8Array
    const totalLen = mp3Data.reduce((acc, buf) => acc + buf.length, 0);
    const mp3Result = new Uint8Array(totalLen);
    let offset = 0;
  ...
  async function fetchWithProgress(url, signal, label, knownSize, onProgress) {
    ...
    try {
      ...
      if (isChunked) {
        if (probeRes.totalSize) {
          size = probeRes.totalSize;
        } else {
        ...
        if (size > 0) {
          ...
          logToPopup(`Chunked Range download supported (unknown size) for ${label}. Downloading dynamically...`);
          const chunks = [];
          let totalBytes = 0;
          let start = 0;
          while (true) {
            ...
            if (chunkFailed || !arrayBuf || arrayBuf.byteLength === 0) {
              break;
            }
            ...
            onProgress(Math.round(totalBytes / 1048576)); // Show MB downloaded as progress
  ...
  async function triggerDownloadMux(videoUrl, videoSize, audioUrl, audioSize, subtitleUrl, title, ext, thumbnail) {
    let cleanTitle = (title || 'video').replace(/[\\/:*?"<>|]/g, '_');
    // Strip existing video/audio extensions to prevent double extension (e.g., video.mp4.mp3)
    cleanTitle = cleanTitle.replace(/\.(mp4|webm|m4a|mp3|3gp|ogg|aac|flac|wav)$/i, '');
    ...
    showToast(taskId, filename, thumbnail);
    updateToastProgress(0, 'Starting download...');
    ...
    try {
      ...
      if (videoUrl) {
        videoBuffer = await fetchWithProgress(videoUrl, controller.signal, "media stream", videoSize, (pct) => {
          const factor = audioUrl ? 0.5 : (isMp3 ? 0.5 : 0.9);
          const rounded = Math.round(pct * factor);
          updateToastProgress(rounded, `Downloading... ${rounded}%`);
          if (extensionAPI && extensionAPI.runtime) {
            extensionAPI.runtime.sendMessage({
              action: 'update_progress_broadcast',
              taskId: taskId,
      ...
      if (audioUrl) {
        ...
        for (let i = 0; i < audioTracksToFetch.length; i++) {
          ...
          const trackBuf = await fetchWithProgress(track.url, controller.signal, `audio track (${track.name})`, track.size, (pct) => {
            const baseProgress = videoUrl ? 50 : 0;
            const weight = (videoUrl ? 40 : 50) / audioTracksToFetch.length;
            const currentProgress = baseProgress + Math.round((i * weight) + (pct * weight / 100));
            const capped = Math.min(90, currentProgress);
            updateToastProgress(capped, `Downloading audio... ${Math.round(pct)}%`);
            if (extensionAPI && extensionAPI.runtime) {
              extensionAPI.runtime.sendMessage({
                action: 'update_progress_broadcast',
                taskId: taskId,
      ...
      const isAudioOnly = (ext.toLowerCase() === 'mp3' || ext.toLowerCase() === 'm4a');
      const actualExt = (videoBuffer && !isAudioOnly) ? detectedVideoFormat : ext;
      if (actualExt !== ext) {
      ...
      if (videoBuffer && (audioBuffers.length > 0 || subtitleTracksData.length > 0)) {
        ...
        updateToastProgress(92, 'Muxing tracks...');
        if (extensionAPI && extensionAPI.runtime) {
          extensionAPI.runtime.sendMessage({
            action: 'update_progress_broadcast',
            taskId: taskId,
      ...
      // ─── MP3 Conversion ─────────────────────────────────────────────────────
      if (isMp3 && finalData) {
        // Convert M4A/WebM audio buffer to MP3 using lamejs
        logToPopup('Starting MP3 conversion via lamejs...');
        updateToastProgress(50, 'Converting to MP3...');
        if (extensionAPI && extensionAPI.runtime) {
          extensionAPI.runtime.sendMessage({
            action: 'update_progress_broadcast',
            taskId: taskId,
        ...
        try {
          const mp3Data = await convertM4aToMp3(finalData.buffer, controller.signal, (pct) => {
            const mapped = 50 + Math.round(pct * 0.45);
            updateToastProgress(mapped, `Converting to MP3... ${pct}%`);
            if (extensionAPI && extensionAPI.runtime) {
              extensionAPI.runtime.sendMessage({
                action: 'update_progress_broadcast',
                taskId: taskId,
          ...
          finalData = mp3Data;
          logToPopup(`MP3 conversion done. Size: ${(mp3Data.byteLength / 1024 / 1024).toFixed(2)} MB`);
        } catch (mp3Err) {
          logToPopup(`MP3 conversion failed: ${mp3Err.message}. Falling back to m4a.`);
          filename = filename.replace(/\.mp3$/i, '.m4a');
          // finalData stays as the original m4a
        }
      }
      // ────────────────────────────────────────────────────────────────────────
      ...
      updateToastProgress(99, 'Finalizing...');
      if (extensionAPI && extensionAPI.runtime) {
        extensionAPI.runtime.sendMessage({
          action: 'update_progress_broadcast',
          taskId: taskId,
        ...
        try {
          ...
          const fallbackTimer = setTimeout(() => {
            if (!resolved) {
              resolved = true;
              logToPopup(`downloads_api timeout, falling back to local anchor click...`);
              triggerAnchorClickDownload(finalData, filename);
              finishToast(true, 'Download complete!');
              if (extensionAPI && extensionAPI.runtime) {
                extensionAPI.runtime.sendMessage({
      ...
      const isAborted = (err.name === 'AbortError' || err.message === 'Aborted' || err.message?.toLowerCase().includes('abort'));
      finishToast(false, isAborted ? 'Cancelled' : 'Download failed');
      if (extensionAPI && extensionAPI.runtime) {
        extensionAPI.runtime.sendMessage({
          action: 'remove_active_download',
          taskId: taskId,
  ...
  function triggerAnchorClickDownload(dataBytes, filename, directUrl) {
    let url;
    if (directUrl) {
      url = directUrl;
    } else {
