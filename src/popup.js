...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.screen.width < 600;
  if (isMobile) {
  ...
  function applyTheme(theme, pitchBlack = false) {
    ...
    if (theme === 'dark' || theme === 'pitch-black') {
      ...
      if (pitchBlack || theme === 'pitch-black') {
  ...
  chrome.storage.local.get([
    ...
    const isPitch = res.theme === 'pitch-black' || res.pitchBlack === true;
    const actualTheme = (res.theme === 'pitch-black') ? 'dark' : (res.theme || 'light');
    applyTheme(actualTheme, isPitch);
  ...
  if (settingTheme) {
    settingTheme.addEventListener('change', (e) => {
      ...
      const actualTheme = isPitch ? 'dark' : themeVal;
      chrome.storage.local.set({ theme: themeVal, pitchBlack: isPitch }, () => {
        applyTheme(actualTheme, isPitch);
      });
    });
  }
